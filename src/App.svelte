<script lang="ts">
  ...
  const dataSourceIcon = $derived(
    dataSource === 'local' ? '\u{1F7E2}' : dataSource === 'cloud' ? '\u2601\uFE0F' : '\u{1F440}'
  );
  ...
  <div class="flex h-screen overflow-hidden bg-slate-900">
    ...
    <aside class="flex flex-col border-r border-slate-700/50 bg-slate-950 transition-all duration-200 {sidebarCollapsed ? 'w-16' : 'w-56'}">
      <div class="flex h-14 items-center gap-2 border-b border-slate-700/50 px-4">
        ...
        {:else}
          <span class="text-lg font-bold text-emerald-400">M</span>
        {/if}
      </div>

      <nav class="flex-1 space-y-1 p-2">
        {#each navItems as item}
          <Link to={item.path} let:active>
            <div class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition-colors {active ? 'bg-slate-800 text-emerald-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200'}">
    ...
    <div class="flex flex-1 flex-col overflow-hidden">
      ...
      <main class="flex-1 overflow-auto p-6">
        <Route path="/" component={Dashboard as any} />
        <Route path="/patterns" component={Patterns as any} />
        <Route path="/patterns/new" component={NewPattern as any} />
        <Route path="/patterns/:id" let:params>
          <PatternEditor id={params.id} />
        </Route>
      </main>
    </div>
  </div>
